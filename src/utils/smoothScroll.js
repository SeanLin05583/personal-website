...
export default function smoothScroll(targetY, ref) {
  ...
  // 移動的距離
  const movingSpacing = Math.abs(targetElement.scrollY - targetY);
  ...
  // 移動方向 往下滑為正 往上滑為負
  const directionBase = (targetY - targetElement.scrollY) > 0 ? 1 : -1;
  ...
  const scrollInterval = setInterval(() => {
    let scrollToY = ref ? targetElement.scrollTop : targetElement.scrollY;
